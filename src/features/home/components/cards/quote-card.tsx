    ...
    <button onClick={() => console.log("Ver más")} className="w-full border max-w-xs sm:w-72 h-auto sm:h-80 bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 sm:hover:shadow-xl relative group">
      ...
      <div className="flex flex-col items-center sm:justify-center h-full p-6 relative z-20">
        <div className="transition-all duration-300 transform sm:group-hover:-translate-y-[60%] flex flex-col items-center">
          <div className="w-36 h-36 group-hover:w-24 group-hover:h-24 bg-blue-100 rounded-full flex items-center justify-center mb-1 transition-transform duration-300">
            <Image
              src={image}
              width={110}
              height={110}
              className="object-cover group-hover:w-[80px]"
              alt={`Imagen de ${title}`}
            />
          </div>
      ...
      <div className="absolute top-0 -right-2.5 w-20 h-20 overflow-hidden transition-opacity duration-300 sm:group-hover:opacity-0">
