...
import { DeductiblesAccordionProps, Member } from "../../types"


const OPTION_LABELS: { [key: string]: string } = {
  opcion_2: "Deducible (0-44 años)",
  opcion_4: "Deducible (45+ años)",
}

const DEDUCTIBLE_LABELS: { [key: string]: string } = {
...
export function DeductiblesAccordion({
  deductiblesData,
  additionalInfo,
  protectWho,
  mainAge = 0
}: DeductiblesAccordionProps) {
  if (!deductiblesData || !additionalInfo) return null;
  ...
  return (
    <Accordion type="single" collapsible className="w-full">
      {Object.entries(deductiblesData).map(([optionKey, deductibles]) => {
          ...
          <AccordionItem key={optionKey} value={optionKey}>
            ...
            <AccordionContent>
              <div className="space-y-2 text-sm bg-muted/50 rounded-lg p-3 mb-3">
                {Object.entries(deductibles).map(([deductibleKey, value]) => (
                  <div key={deductibleKey} className="flex justify-between items-center py-1">
                    <span className="text-muted-foreground">{DEDUCTIBLE_LABELS[deductibleKey]}:</span>
                    <span className="font-medium text-primary">{formatCurrency(value)}</span>
                  </div>
                ))}
              </div>
